...
SDP.prototype.getMediaSsrcMap = function () {
    ...
    for (let mediaindex = 0; mediaindex < this.media.length; mediaindex++) {
        const mid = SDPUtil.parseMID(SDPUtil.findLine(this.media[mediaindex], 'a=mid:'));
        const media = {
            mediaindex,
            mid,
            ssrcs: {},
    ...
    for (let i = 0; i < this.media.length; i++) {
        ...
        if (mline.media === 'audio' || mline.media === 'video') {
            ...
            for (let j = 0; j < mline.fmt.length; j++) {
                const rtpmap = SDPUtil.findLine(this.media[i], `a=rtpmap:${mline.fmt[j]}`);
                elem.c('payload-type', SDPUtil.parseRTPMap(rtpmap));
                // put any 'a=fmtp:' + mline.fmt[j] lines into <param name=foo
                // value=bar/>
                const afmtpline = SDPUtil.findLine(this.media[i], `a=fmtp:${mline.fmt[j]}`);
                if (afmtpline) {
            ...
            if (ssrc) {
                ...
                const ssrcGroupLines = SDPUtil.findLines(this.media[i], 'a=ssrc-group:');
            ...
            if (ridLines.length && browser.usesRidsForSimulcast()) {
                ...
                const unifiedSimulcast = SDPUtil.findLine(this.media[i], 'a=simulcast:');
                if (unifiedSimulcast) {
                    elem.c('rid-group', {
                        semantics: 'SIM',
                        xmlns: 'urn:xmpp:jingle:apps:rtp:ssma:0'
